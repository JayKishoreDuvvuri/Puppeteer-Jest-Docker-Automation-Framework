import { colourBlack, printedSummerDress, imagePicture, colourOrange, colourBlue, colourYellow, colourSelected, headerLogo, quantityWanted, numberOfItems, checkout, productPrice } from '../utils/locators';
...
class ProductPage extends BasePage {
	...
	async blackColour() {
		...
		const result = await page.evaluate('document.querySelector("#color_11").getAttribute("class")');
		return result;
	}

	async blackColourImageUpdated() {
		await this.isElementVisible(imagePicture);
		const result = await page.evaluate('document.querySelector("#bigpic").getAttribute("src");');
		return result;
	}

	async orangeColour() {
		await this.waitAndClick(colourOrange);
		const result = await page.evaluate('document.querySelector("#color_13").getAttribute("class")');
		return result;
	}
	...
	async blueColour() {
		...
		const result = await page.evaluate('document.querySelector("#color_14").getAttribute("class")');
		return result;
	}
	...
	async yellowColour() {
		...
		const result = await page.evaluate('document.querySelector("#color_16").getAttribute("class")');
		return result;
	}
